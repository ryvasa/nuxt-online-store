<template>
  <div
    :class="[
      ...
      isAtTop && path === '/' ? 'lg:bg-transparent bg-base-200' : 'bg-base-200',
    ]"
  >
    <div class="flex-1">
      <nuxt-link to="/" class="btn btn-ghost text-xl"
        ><p :class="[isAtTop && path === '/' && 'lg:text-base-200']">
    ...
    <div class="flex-none gap-4 lg:gap-2">
      <div class="lg:flex hidden">
        <input
          ...
          :class="[
            ...
            isAtTop &&
              path === '/' &&
              'text-base-200 placeholder:text-base-200',
        ...
        <label for="search" class="btn btn-ghost btn-circle btn-sm lg:btn-md">
          <svg
            :class="[isAtTop && path === '/' && 'text-base-200']"
      ...
      >
        <div class="indicator">
          <svg
            :class="[isAtTop && path === '/' && 'lg:text-base-200']"
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5"
            fill="none"
            viewBox="0 0 24 24"
      ...
      <label for="cart" class="btn btn-ghost btn-circle btn-sm lg:btn-md">
        <div class="indicator">
          <svg
            :class="['h-5 w-5', isAtTop && path === '/' && 'lg:text-base-200']"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
      ...
      <div class="dropdown dropdown-end">
        ...
        >
          ...
          <li>
            <nuxt-link to="/transaction">Order Transaction</nuxt-link>
          </li>
          <li class="lg:hidden">
...
// Watch for route changes to update the path
watch(
  () => route.path,
  (newPath: string) => {
    path.value = newPath;
  },
  { immediate: true } // This ensures that the path is set on initial load
);
